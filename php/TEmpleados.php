...
<label2>Tabla de Empleados</label2>
...
if (isset($_POST['busqueda']) && !empty($_POST['busqueda'])) {
    ...
    $sql = "SELECT * FROM tusuarios WHERE TDeptos_id LIKE '%$busqueda%' OR id = '$busqueda' OR TDetallesEmpleados_id = '$busqueda' OR TSesion_id = '$busqueda' OR TRoles_id = '$busqueda' OR TVentasConcentrado_id = '$busqueda'";
}
...
if ($result->num_rows > 0) {
    echo "<table>";
    echo "<tr><th>ID</th><th>TDeptos_id</th><th>TDetallesEmpleados_id/Kg</th><th>TSesion_id</th><th>TRoles_id</th><th>TVentasConcentrado_id</th><th colspan=2>Operaciones</th></tr>";

    while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . $row["id"] . "</td>";
        echo "<td>" . $row["TDeptos_id"] . "</td>";
        echo "<td>" . $row["TDetallesEmpleados_id"] . "</td>";
        echo "<td>" . $row["TSesion_id"] . "</td>";
        echo "<td>" . $row["TRoles_id"] . "</td>";
        echo "<td>" . $row["TVentasConcentrado_id"] . "</td>";
